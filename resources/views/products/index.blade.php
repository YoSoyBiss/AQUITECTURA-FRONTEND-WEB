...
<head>
    ...
    <style>
        ...
        th { background-color: #d7ccc8; color: #4e342e; }
        ...
        .thumb-preview:hover .zoom-overlay,
        .thumb-preview:focus .zoom-overlay,
        .thumb-preview:focus-visible .zoom-overlay {
            opacity: 1;
        }
        .zoom-overlay svg {
    ...
    <table>
        ...
        @forelse($products as $p)
            @php
                ...
                // Arreglo de imágenes original
                $images = collect($p['images'] ?? [])->values()->all();

                // Buscar índice de imagen principal
                $mainIndex = collect($images)->search(fn($x) => !empty($x['is_main']));
                if ($mainIndex === false) { $mainIndex = 0; }

                // URL/ALT de la imagen a mostrar en la celda
                $mainItem = $images[$mainIndex] ?? null;
                $mainUrl  = $mainItem['url'] ?? null;
                $mainAlt  = $mainItem['alt'] ?? 'Imagen';
            @endphp
            <tr>
                <td>{{ $p['id'] }}</td>

                <!-- Celda: preview único con lupa -->
                <td>
                    @if(!$images || !$mainUrl)
                        <div class="thumb-preview" tabindex="0"
                             data-images='@json($images)'
                             data-start="0"
                             onclick="openGalleryFromPreview(this)"
                             onkeydown="previewKeyOpen(event,this)">
                            <img src="https://via.placeholder.com/96x96?text=Sin+Imagen" alt="Sin imagen">
                            <div class="zoom-overlay" aria-hidden="true">
                                <!-- Lupa (SVG) -->
                                <svg viewBox="0 0 24 24" fill="#fff" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ver imagen">
                                  <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 5 1.5-1.5-5-5zM10 14a4 4 0 110-8 4 4 0 010 8z"/>
                                </svg>
                            </div>
                        </div>
                    @else
                        <div class="thumb-preview" tabindex="0"
                             data-images='@json($images)'
                             data-start="{{ $mainIndex }}"
                             onclick="openGalleryFromPreview(this)"
                             onkeydown="previewKeyOpen(event,this)">
                            <img src="{{ $mainUrl }}" alt="{{ $mainAlt }}"
...
<script>
    ...
    function confirmDelete() {
        ...
        form.action = "{{ url('/products') }}/" + deleteId;
        form.submit();
    }
    ...
    // Nuevo: abrir desde el preview único
    function openGalleryFromPreview(el){
        const images = JSON.parse(el.dataset.images || '[]');
        if (!images.length) return;
        const start = parseInt(el.dataset.start || '0', 10) || 0;

        LB.images = images;
        LB.index = Math.max(0, Math.min(start, images.length - 1));
        renderLB();
        document.getElementById('lb').style.display = 'block';
        document.addEventListener('keydown', lbKeyHandler);
    }

    // Accesibilidad: abrir con Enter o Espacio
    function previewKeyOpen(evt, el){
        if (evt.key === 'Enter' || evt.key === ' ') {
            evt.preventDefault();
            openGalleryFromPreview(el);
        }
    }
    ...
    function backdropClose(e){
        if (e.target.id === 'lb') closeGallery();
    }
    ...
    function renderLB(){
        ...
        imgEl.src = it.url || 'https://via.placeholder.com/800x600?text=Sin+Imagen';
